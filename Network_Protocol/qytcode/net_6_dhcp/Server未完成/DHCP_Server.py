        ...
        if pkt.getlayer(DHCP) and pkt.getlayer(DHCP).fields['options'][0][1] == 1:
            ...
            sendp(
                ...
                BOOTP(
                    ...
                    options=b'c\x82Sc') /
                DHCP(options=[('message-type', 2), ('subnet_mask', '255.255.255.0'), ('server_id', self.server_ip),
        ...
        if pkt.getlayer(DHCP) and pkt.getlayer(DHCP).fields['options'][0][1] == 3:
            ...
            sendp(
                ...
                BOOTP(
                    ...
                    chaddr=pkt.getlayer(BOOTP).fields['chaddr'] + b'\x00' * 10,
                    options=b'c\x82Sc') /
                DHCP(options=[('message-type', 5), ('renewal_time', 345600), ('rebinding_time', 604800),
                              ('lease_time', 691200), ('server_id', self.server_ip), ('subnet_mask', '255.255.255.0'),
                              (81, b'\x00\xff\xff'), ('domain', b'cjd.com\x00'), ('router', '192.168.1.254'), 'end',
                              'pad', 'pad', 'pad', 'pad', 'pad'])
            )
            print("Sending Ack.\n\nCtrl+C退出。\n")
        sleep(3)
